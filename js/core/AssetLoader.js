...
const attacks=[]; // GitHub Pages fallback: ataque usa os frames-base com movimento procedural
...
const GROUPS={
  common:[...player,...playerWeapons,...Object.values(zombies).flat(),...deaths,...attacks,...items,...effects,...sharedProps],
...
export class AssetLoader{
  ...
  death(t,i){return this.get(`assets/zombies/${t}/death/frame-${i%4}.webp`)}attackZombie(t,i){return this.zombie(t,i)}
  prop(n){return n.startsWith('interior:')?this.get(`assets/props/interiors/${n.slice(9)}.webp`):this.get(`assets/props/${n}.webp`)}farm(state){return this.get(`assets/props/farm/garden-${state}.webp`)}item(n){return this.get(`assets/items/${n}.webp`)}effect(n,i){const fb={shotgun:'heavy',rifle:'pistol'};return this.get(`assets/effects/${fb[n]||n}/frame-${i%4}.webp`)}npc(n,i){return this.get(`assets/npcs/${n}/frame-${i%4}.webp`)}base(level){return this.get(`assets/buildings/base-level-${level}.webp`)}tile(name){return this.get(`assets/tiles/${name}.webp`)}
